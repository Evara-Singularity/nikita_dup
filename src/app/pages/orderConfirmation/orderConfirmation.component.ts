...
export class OrderConfirmationComponent implements OnInit {
    ...
    userType;
    couponCodeData;
    constructor(private localStorageService: LocalStorageService, public _dataService: DataService, @Inject(PLATFORM_ID) platformId, private _ocs: OrderConfirmationService, private _lss: LocalStorageService, private _las: LocalAuthService, private _cartService: CartService, private _router: Router, private footerService: FooterService, private _activatedRoute: ActivatedRoute, private _orderService: OrderConfirmationService) {
    ...
    ngOnInit() {
        ...
        if (this.isBrowser) {
            this._orderService.sendTrackerOrderConfirmationCall({
        ...
        if (this.isBrowser) {
            if (utm_medium) {
                ...
                if (utm_medium === 'mysmartprice') {
                    this.afType = 'iFrame';
                    this.iFrameUrl = 'https://mspdv.in/p.ashx?o=10&e=16&p=' + this.queryParams['transactionAmount'] + '&t=' + this.queryParams['orderId'] + '&udid=' + this.queryParams['transactionAmount'];
                    data['affiliateId'] = 1;
                } else if (utm_medium === 'affprog-mopm') {
                    data['affiliateId'] = 2;
                    let src = 'https://track.in.omgpm.com/916096/transaction.asp?APPID=' + this.queryParams['orderId'] + '&MID=916096&PID=17423&status=' + this.queryParams['transactionAmount'];
                    this._ocs.cs(src, 'script');
                } else if (utm_medium === 'cuelinks_desidime') {
                    this.afType = 'image';
                    this.iFrameUrl = 'https://paritycube.go2cloud.org/SLCD?adv_sub=' + this.queryParams['orderId'] + '&amount=' + this.queryParams['transactionAmount'];
                    data['affiliateId'] = 7;
                } else if (utm_medium === 'clickonik') {
                    const e = {
                        ce: 'img',
                        src: 'https://track.clickonik.com/pixel?adid=5f7ad7f126f6cb33c836f693&&sale_amount=' + this.queryParams['transactionAmount'] + '&sub1=' + this.queryParams['orderId']
                    };
                    this._ocs.cns(e);
                    data['affiliateId'] = 8;
                    ...
                    data["affiliateId"] = 10;
                    this.afType = "iFrame";
                    this.iFrameUrl = "https://tracking.icubeswire.co/aff_a?offer_id=375&adv_sub1=" + this.queryParams["orderId"] + "&sale_amount=" + this.queryParams['transactionAmount'] + this.queryParams['orderId'] + '&amount=' + this.queryParams['transactionAmount'];
                    this.id = "pixelcodeurl";
                }

                // this._lss.clear('utm_medium');
                this._ocs.addAffiliateOrder(data).subscribe(() => {
                    // console.log('Affiliate channel fired and stored data into DB');
                });
            }
            ...
            if (userSession && userSession.authenticated && userSession.authenticated == 'true') {
                let criteoItem = [];
                let couponId = [];
                let prodIds = '', prodNames = '', prodPrices = '', prodQuantities = '', aCat1 = '', aCat2 = '', aCat3 = '', aprodIds = '', aprodNames = '', aprodPrices = '', aprodQuantities = '', aShipping = '', aOffer = '', aTotalDiscount = 0, aTotalShipping = 0, aTotalQuantity = 0;
                ...
                this._cartService.getCartBySession(params).subscribe((cartSession) => {
                    try {
                        ...
                        dataLayer.push({ 'event': 'pr-confirmation', 'ecomm_prodid': google_tag_params.ecomm_prodid, 'ecomm_pagetype': google_tag_params.ecomm_pagetype, 'ecomm_totalvalue': google_tag_params.ecomm_totalvalue, 'google_tag_params': google_tag_params });
                        this._lss.clear('utm_medium');
                        ...
                        try {
                            ...
                            this.sendClickStreamDataError('order_completed_adobe_error', adobeError)
                        }

                        this.sendClickStreamData(cartSession);

                    } catch (trackError) {
                        this.sendClickStreamDataError('order_completed_data_error', trackError)
                    }
                }, reponseError => {
                    this.sendClickStreamDataError('order_completed_api_error', reponseError)
                });
