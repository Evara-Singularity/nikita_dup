...
import {
  AddressListActionModel,
  AddressListModel,
  CountryListModel,
  CreateEditAddressModel,
} from "@app/utils/models/shared-checkout.models";
...
export class QuickOrderAllAddressComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @Output("emitDeliveryAddressSelectEvent$")
  emitDeliveryAddressSelectEvent$: EventEmitter<any> = new EventEmitter<any>();
  @Output("emitBillingAddressSelectEvent$")
  emitBillingAddressSelectEvent$: EventEmitter<any> = new EventEmitter<any>();
  @Output("emitInvoiceTypeEvent$") emitInvoiceTypeEvent$: EventEmitter<string> =
  ...
  ngOnInit() {
    ...
    const INVOICE_TYPE =
      this._cartService.invoiceType && this.isCheckoutModule
        ? this._cartService.invoiceType
        : this.INVOICE_TYPES.RETAIL;
  ...
  ngAfterViewInit(): void {
    if (this.addDeliveryOrBilling) {
      this.triggerDeliveryOrBillingSubscription =
        this.addDeliveryOrBilling.subscribe((addressType: string) => {
  ...
  fetchCountryList() {
    this._addressService
      .getCountryList()
      .subscribe((countryList: CountryListModel[]) => {
  ...
  updateInvoiceType(isGST: boolean) {
    ...
    this.invoiceType.patchValue(
      isGST ? this.INVOICE_TYPES.TAX : this.INVOICE_TYPES.RETAIL
    );
  ...
  updateAddressTypes(userId: number, type: string) {
    ...
    this._addressService
      ...
      .subscribe((response: AddressListModel) => {
  ...
  async displayAddressListPopup(addressType: string) {
    ...
    this.addressListCloseSubscription = (
      this.addressListInstance.instance["emitCloseEvent$"] as EventEmitter<any>
    ).subscribe((actionInfo: AddressListActionModel) => {
      this.closeAddressListPopup();
    });
    this.addressListActionSubscription = (
      this.addressListInstance.instance["emitActionEvent$"] as EventEmitter<any>
    ).subscribe((actionInfo: AddressListActionModel) => {
  ...
  async displayAddressListBillingPopup(addressType: string) {
    ...
    const factory = this.cfr.resolveComponentFactory(AddressListComponent);
    this.addressListBillingInstance =
      this.addressListBillingRef.createComponent(factory, null, this.injector);
    const IS_DELIVERY = addressType === this.ADDRESS_TYPES.DELIVERY;
    let ADDRESSES = null;
    let cIdAddress = null;
    if (IS_DELIVERY) {
    ...
    this.addressListBillingInstance.instance["parentModule"] =
      this.parentModule;
    ...
    this.addressListCloseSubscription = (
      this.addressListBillingInstance.instance[
        "emitCloseEvent$"
      ] as EventEmitter<any>
    ).subscribe((actionInfo: AddressListActionModel) => {
      this.closeAddressListBillingPopup();
    });
    this.addressListActionBillingSubscription = (
      this.addressListBillingInstance.instance[
        "emitActionEvent$"
      ] as EventEmitter<any>
    ).subscribe((actionInfo: AddressListActionModel) => {
  ...
  async displayAddressFormPopup(addressType: string, address) {
    ...
    verifiedPhones = SharedCheckoutAddressUtil.getVerifiedPhones(
      this.USER_SESSION,
      this.deliveryAddressList
    );
    if (addressType === this.ADDRESS_TYPES.DELIVERY) {
      const { CreateEditDeliveryAddressComponent } = await import(
        "./../create-edit-delivery-address/create-edit-delivery-address.component"
      ).finally(() => {});
      ...
      const { CreateEditBillingAddressComponent } = await import(
        "./../create-edit-billing-address/create-edit-billing-address.component"
      ).finally(() => {});
      factory = this.cfr.resolveComponentFactory(
    ...
    this.createEditAddressInstance = this.addressListRef.createComponent(
      factory,
      null,
      this.injector
    );
    this.createEditAddressInstance.instance["isAddMode"] = !address;
    this.createEditAddressInstance.instance["invoiceType"] =
    ...
    this.createEditAddressSubscription = (
      this.createEditAddressInstance.instance[
        "closeAddressPopUp$"
      ] as EventEmitter<any>
    ).subscribe((response: CreateEditAddressModel) => {
  ...
  updateAddressAfterAction(addressType, addresses, isEditMode) {
    ...
    const IS_DELIVERY = addressType === this.ADDRESS_TYPES.DELIVERY;
    const SEPARATED_ADDRESS: AddressListModel =
      this._addressService.separateDeliveryAndBillingAddress(addresses);
    ...
    if (this.addressListInstance) {
      this.addressListInstance.instance["addresses"] = IS_DELIVERY
        ? this.deliveryAddressList
        : this.billingAddressList;
    }
  ...
  closeAddressListPopup() {
    ...
    console.log(
      "addressListInstance",
      this.addressListInstance,
      this.addressListRef.element.nativeElement
    );
  }
  ...
  get displayBillingAddresses() {
    return this._displayBillingAddresses;
  }

  set displayBillingAddresses(type) {
    this._displayBillingAddresses = type;
  }

  get isGSTUser() {
    return this._isGSTUser;
  }

  set isGSTUser(type: boolean) {
    this._isGSTUser = type;
  }
  get isCheckoutModule() {
    return this.parentModule === "Checkout";
  }

  getLabel(label) {
    return label == "Billing" ? " GST" : " ADDRESS";
  }
  ...
  ngOnDestroy(): void {
    ...
    if (this.addressListCloseSubscription)
      this.addressListCloseSubscription.unsubscribe();
    if (this.addressListCloseSubscription)
      this.addressListCloseSubscription.unsubscribe();
    if (this.createEditAddressSubscription)
      this.createEditAddressSubscription.unsubscribe();
    if (this.triggerDeliveryOrBillingSubscription)
      this.triggerDeliveryOrBillingSubscription.unsubscribe();
  }
}
