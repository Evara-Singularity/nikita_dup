...
export class ViewComponent {
  ...
  ngOnInit() {
    ...
    if (this.isBrowser) {
      if ((<HTMLElement>document.querySelector(".toggle-head ")))
        (<HTMLElement>document.querySelector(".toggle-head ")).style.display = 'none';
    }

    this.getClustorCategoryApi().subscribe((res) => {
      console.log(res);
      if (res['status'] && res['data']) {
        this.clusterCategoryData = res['data'][0]['block_data']['all-categories'];
      }
    });
    ...
    this._meta.addTag({ "name": "description", "content": "List of all categories of Moglix.com" });
    this._meta.addTag({ "name": "og:description", "content": "List of all categories of Moglix.com" });
  ...
  getAllCategoriesData() {
    this.categoriesDataService.getCategoriesData().subscribe(
      (response) => {
  ...
  goto(level, index?) {
    ...
    if (level > 0) {
      ...
      for (let i = 0; i < indices.length; i++) {
        if (this.categoriesData[indices[i]] && this.categoriesData[indices[i]].childCategories) {
