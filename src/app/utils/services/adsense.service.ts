...
export class AdsenseService {
  ...
  getAdsense(
    ...
    return this._getAdsenseCall(categoryId, brandName, msn).pipe(
      map((response) => {
  ...
  private _mapResponse(response) {
    if (
      response &&
      response["data"] &&
      response["data"]["status"] == this.STATUS_RUNNING
    ) {
  ...
  private _mapfeaturedProductUnit(campaignData): {
    FEATURED_PRODUCT_ADS?: any;
  } {
    if (
      ...
      return {
        FEATURED_PRODUCT_ADS: campaignData["featuredProductUnit"]["msns"],
      };
    }
    return {};
  }
  ...
  private _promotedBrandUnit(campaignData) {
    if (
      campaignData["promotedBrandUnit"] &&
      campaignData["promotedBrandUnit"]["banners"] &&
      campaignData["promotedBrandUnit"]["banners"].length > 0
    ) {
      const PROMOTOTED_BRAND_ADS = (
        campaignData["promotedBrandUnit"]["banners"] as PromotedBrandAd[]
      ).map((ads) => {
        return {
          id: ads["id"],
  ...
  private _videoUnit(campaignData) {
    if (
      campaignData["videoUnit"] &&
      campaignData["videoUnit"]["banners"] &&
      campaignData["videoUnit"]["banners"].length > 0
    ) {
      const VIDEOS_ADS = (
        campaignData["videoUnit"]["banners"] as VideoAdUnit[]
      ).map((ads) => {
        return {
          id: ads["id"],
