...
const ALPBRK: any = makeStateKey<{}>("ALPBRK");         //Attrtibute Listing Result Key
...
export class CategoryComponent implements OnInit, AfterViewInit {
    ...
    constructor(@Optional() @Inject(RESPONSE) private _response, private _tState: TransferState, private _renderer2: Renderer2,
        ...
        private _commonService: CommonService, private _categoryService: CategoryService, private _pageScrollService: PageScrollService, private ngxSiemaService: NgxSiemaService) {
    ...
    ngOnInit() {
        ...
        this.combineLatestUnsub = combineLatest(this._activatedRoute.params, this._activatedRoute.queryParams, this._activatedRoute.fragment).subscribe(data => {
            ...
            if (this._tState.hasKey(ALPARK)) {
                ...
                this._categoryService.getCIMSAttributeListingInfo(data[0]['attribute']).subscribe((response) => {
    ...
    fetchCIMSRelatedData(categoryCode, defaultParams) {
        this.forkJoinUnsub = forkJoin([this.getRelatedCategories(categoryCode).pipe(map(res => res)), this.refreshProducts(defaultParams).pipe(map(res => res))])
            .subscribe((res) => {
    ...
    createDefaultParams(defaultApiParams) {
        ...
        if (defaultApiParams['str']) {                             //api params/filters
