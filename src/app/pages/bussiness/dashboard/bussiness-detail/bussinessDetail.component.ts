...
export class BussinessDetailComponent implements OnDestroy {
    businessDetail: { companyName: string, gstin: string, isGstInvoice: boolean, pan: string };
    ...
    constructor(private _localAuthService: LocalAuthService, private meta: Meta, @Inject(PLATFORM_ID) platformId, private _cartService: CartService, private _localStorageService: LocalStorageService, private _formBuilder: FormBuilder, private _businessDetailService: BusinessDetailService, private tms: ToastMessageService) {
    ...
    ngOnInit() {
        ...
        this.businessDetail = { companyName: "", pan: "", gstin: "", isGstInvoice: user.userType == globalConstant.userType.business ? false : true };
        ...
        this.businessDetailForm = this._formBuilder.group(
            {
                ...
                postCode: [null, [Validators.required, Validators.minLength(6), SpaceValidator.validateSpace, Step.validatePostCode]],
                address: this._formBuilder.group({
                    idAddress: [null],
                    addressCustomerName: [null],
                    postCode: [null],
                    landmark: [null],
                    addressLine: [null, [Validators.required, Validators.pattern(/^[a-zA-Z0-9._;, \/()-]*$/)]],
                    city: [null, [Validators.required, Validators.pattern('^([a-zA-Z0-9_]*[ \t\r\n\f]*[\-\,\/\.\(\)]*)+')]],
                    idCountry: [110],
    ...
    getBusinessDetail() {
        ...
        this._businessDetailService.getBusinessDetail(data).subscribe(res => {
            ...
            if (data) {
                ...
                this.businessDetailForm.controls['companyName'].setValue(data['companyName']);
                ...
                this.businessDetailForm.controls['isGstInvoice'].setValue(data['isGstInvoice']);
                this.businessDetailForm.controls['postCode'].setValue(data['postCode']);
                if (data['companyName']) {
                    this.companyName.markAsDirty();
                }
                if (data['address']) {
                    this.businessDetailForm.controls["address"]["controls"]['idAddress'].setValue(data['address']['idAddress']);
                    this.businessDetailForm.controls["address"]["controls"]['addressCustomerName'].setValue(data['address']['addressCustomerName']);
                    this.businessDetailForm.controls["address"]["controls"]['postCode'].setValue(data['address']['postCode']);
                    this.businessDetailForm.controls["address"]["controls"]['landmark'].setValue(data['address']['landmark']);
                    this.businessDetailForm.controls["address"]["controls"]['addressLine'].setValue(data['address']['addressLine']);
                    this.businessDetailForm.controls["address"]["controls"]['city'].setValue(data['address']['city']);
                    this.businessDetailForm.controls["address"]["controls"]['idCountry'].setValue(data['address']['idCountry']);
                    this.businessDetailForm.controls["address"]["controls"]['idState'].setValue(data['address']['idState'] ? data['address']['idState'] : "");
                    this.businessDetailForm.controls["address"]["controls"]['gstin'].setValue(data['address']['gstin']);
                    this.businessDetailForm.controls["address"]["controls"]['idCustomer'].setValue(data['address']['idCustomer']);
                    this.businessDetailForm.controls["address"]["controls"]['idAddressType'].setValue(data['address']['idAddressType']);
                    this.businessDetailForm.controls["address"]["controls"]['active'].setValue(data['address']['active']);
                    this.businessDetailForm.controls["address"]["controls"]['invoiceType'].setValue(data['address']['invoiceType']);
    ...
    getCityByPincode() {
        setTimeout(() => {
            ...
            if (this.businessDetailForm.controls['postCode'].valid) {
                this._businessDetailService.getCityByPinCode(this.businessDetailForm.controls['postCode'].value).subscribe(res => {
                    this.isPincodeBusy = false;
                    if (res['status']) {
                        this.businessDetailForm.get("address").get('postCode').setValue(this.businessDetailForm.controls['postCode'].value);
                        this.businessDetailForm.controls["address"]["controls"]['city'].setValue(res['dataList'][0]['city']);
                        this.stateList.forEach(element => {
                            if (element.idState == parseInt(res['dataList'][0]['state'])) {
                                this.businessDetailForm.controls["address"]["controls"]['idState'].setValue(element.idState);
                            }
                        })
                    } else {
                        this.isPinCodeAvailble = false;
                        this.businessDetailForm.controls["address"]["controls"]['city'].setValue(null);
                        this.businessDetailForm.controls["address"]["controls"]['idState'].setValue(null);
                    }
                })
            }
        }, 500)
        ...
        this._businessDetailService.setBusinessDetail(budata).subscribe(
            res => {
                ...
                if (res['statusCode'] === 200) {
                    if (!this.businessDetailForm.controls["address"]["controls"]["idAddress"].value)
                        this.getBusinessDetail();
    ...
    get canSave() {
        return this.isPinCodeAvailble == true && this.businessDetailForm.valid && this.isGSTINVerified;
    }
    ...
    get gstin() { return this.businessDetailForm.get('gstin'); }
    get companyName() { return this.businessDetailForm.get('companyName'); }
    get email() { return this.businessDetailForm.get('email'); }
    get phone() { return this.businessDetailForm.get('phone'); }
    get postCode() { return this.businessDetailForm.get('postCode'); }
    get addressLine() { return this.businessDetailForm.get('address').get('addressLine'); }
    get city() { return this.businessDetailForm.get('address').get('city'); }
    get idState() { return this.businessDetailForm.get('address').get('idState'); }
    get address() { return this.businessDetailForm.get('address') };
    get isGstInvoice() { return this.businessDetailForm.get('isGstInvoice'); }
