			...
			if (!rawData['data']['error']) {
				...
				if (!this.isServer) {
					setTimeout(function () {
						...
						(<HTMLElement>(
							document.querySelector('.fixed_section')
						)).style.minHeight = wh + 'px';
						window.addEventListener(
							...
							function () {
								...
								Array.prototype.forEach.call(getClass, function (el) {
									...
									let d =
										ClientUtility.offset(el).top +
										ClientUtility.outerHeight(el) -
										ClientUtility.outerHeight(el.querySelector('.wp-20'));
									if (bp >= c) {
										el.querySelector('.wp-20').classList.add('block1');
										let w =
											(window.outerWidth -
												(<HTMLElement>document.querySelector('.wp-95'))
													.offsetWidth) /
												2 -
											20;
										let wblock1 =
											(<HTMLElement>document.querySelector('.wp-95'))
												.offsetWidth -
											(<HTMLElement>document.querySelector('.wp-79'))
												.offsetWidth +
											26;
										(<HTMLElement>(
									...
									if (bp >= d) {
										el.querySelector('.wp-20').classList.removeClass('block1');
										(<HTMLElement>el.querySelector('.wp-20')).style.position =
											'absolute';
										(<HTMLElement>el.querySelector('.wp-20')).style.bottom =
											'-10';
										alert(d);
									}
								});
