        ...
        if (this.transferState.hasKey(GET_CIMS_ATTRIBUTE_LISTING)) {// && this.transferState.hasKey(OTHER_DATA)
            ...
            const otherDataObj = this.http.get(cims_attribute_url, requestOptions).pipe(share(), mergeMap((_cimsResponse) =>
            {
                ...
                if (CIMS_DATA && CIMS_DATA['defaultParams']['category']) {
                    ...
                    this.http.get(get_category_code_url + CATEGORY, requestOptions).pipe(mergeMap(catData => this.http.get(breadcrump_url + '&pagetitle=' + _activatedRouteSnapshot.params['attribute'].replaceAll('-', ' ') + '&source=' + catData['categoryDetails']['categoryLink'], requestOptions))),
