...
const BOD = makeStateKey<any>('bestofferdata');
...
export class BestOfferComponent {
  ...
  getBestOfferData(){
    if (this._tState.hasKey(BOD)) {
      this.BestOfferData = this._tState.get(BOD, {data : ""}).data;
      setTimeout(() => {
      ...
      this.dealsService.getBestOfferData({ headerData: {'Content-Type':'text/html'} }).subscribe(
        data=>{
          this.BestOfferData = data;
          if(this.isServer){
            this._tState.set(BOD, {data : "'" + this.BestOfferData + "'"});
          }
          setTimeout(() => {
            // wait for DOM rendering
            this.reinsertLinks();
